...
지연과 연결 재설정 명령은 같은 프록시에 남아 있던 장애 규칙을 먼저 지운 뒤
새 규칙을 만듭니다. `db-down`과 `edge-down`은 프록시만 끄며 기존 규칙은
해당 `up` 명령이 프록시를 다시 열 때 지웁니다. 지연과 연결 재설정을 겹쳐야
한다면 제어 스크립트를 바꾸지 말고 별도의 실험 조건으로 기록해 실행합니다.

PostgreSQL 응답에 300ms 지연과 최대 50ms 지터를 넣거나 연결을 완전히 끊을 수 있습니다.
이 값은 아래 수동 예시의 조건입니다. 자동 복구 시나리오는 단계별 응답을
비교하기 쉽도록 같은 300ms 지연에 지터 `0`을 사용합니다.
...
2. PostgreSQL 응답에 300ms 지연과 지터 `0`을 넣고 준비 상태 응답 시간과 본문을 기록합니다.
...
결과는 JSON으로 표준 출력에 기록됩니다. 각 단계에는 HTTP 상태, 응답 시간,
준비 상태 본문 또는 네트워크 오류가 들어갑니다. `passed`는 예상한 상태 전이가
확인됐다는 뜻입니다. 지연 단계의 실제 응답 시간은 결과 파일에서 설정값과
함께 비교합니다.

저장소의 날짜가 붙은 복구 결과는 읽기 편하게 항목을 정리한 보관 형식이며,
실행기가 내보내는 JSON을 그대로 복사한 schema는 아닙니다. 새 실행 결과를
비교할 때는 `passed`, `steps`, `settings`의 의미를 대응시키고, 필드 이름이
같더라도 같은 schema로 처리하지 않습니다.

`pnpm load:faults`의 대상 URL은 실수로 외부 환경을 건드리지 않도록 루프백
주소만 허용합니다. `toxiproxy-control.mjs`를 단독으로 실행할 때는
`TOXIPROXY_API_URL`에 외부 주소도 지정할 수 있으므로, 검증 전용 Toxiproxy인지
직접 확인해야 합니다. 기본 포트가 다른 경우 아래 환경 변수를 바꿀 수 있습니다.
...
```sh
fnm exec --using=24.18.1 node --test tests/load/fault-scenario.test.mjs
fnm exec --using=24.18.1 node --test tests/load/load-harness.test.mjs
k6 inspect tests/load/pong-load.js
fnm exec --using=24.18.1 node tests/load/toxiproxy-control.mjs plan
...
정적 검증은 부하 기준을 통과했다는 뜻이 아닙니다. 결과를 남길 때는 실행 날짜,
CPU와 메모리, 운영체제, Node·k6·Docker 버전, 환경 변수와 통과 기준 결과를
함께 기록합니다. 실행하지 못한 연결 1,000개 결과나 장애 복구 수치는 빈칸
대신 미측정으로 표시합니다.

Prometheus counter는 API 프로세스가 시작된 뒤 누적됩니다. 같은 프로세스에서
연속으로 부하를 실행하면 이전 연결과 경기 결과가 다음 결과에 섞이므로 시작
값을 차감하거나 새 API와 초기화한 테스트 DB에서 측정합니다.
