...
development/test/production/demo capability, secure cookie, trusted proxy, CORS origin/method/header, production persistence 요구를 하나의 검증된 runtime mode에서 파생하는 과정을 복원합니다.

이 문서는 완성된 해설이 아니라 exact SHA를 순서대로 확인해 구현 발전을 복원하기 위한 scaffold입니다.

### 직접 연결되는 불변식

- 환경별 capability는 검증된 단일 runtime mode에서 파생됩니다.
- production은 영속 저장소 없이 시작하지 않으며 demo의 memory semantics와 혼동되지 않습니다.
- client address를 신뢰하는 기능은 명시적으로 proxy trust를 활성화한 경우에만 forwarded header를 사용합니다.

## 2. 핵심 질문

- runtime mode는 어떤 환경 변수 우선순위로 결정되며 invalid explicit value를 왜 거부합니까?
- secure cookie, development login, guest route, repository selection이 같은 mode source에서 파생됩니까?
- CORS origin/method/header와 `credentials: true` 조합이 browser cookie 경계와 일치합니까?
- proxy trust가 client IP 기반 guest limit과 session address binding에 어떤 영향을 줍니까?
...
| ---: | --- | --- | :---: | --- | --- |
| 1 | `85ac2a949439` | `test(api): 실행 환경 기본값 검증` | B | PROTOCOL, PERSISTENCE, TEST | configuration precedence와 local runtime default를 고정합니다. |
| 2 | `66155cf8a27d` | `fix(api): 변경 요청용 CORS method와 header 허용` | B | AUTH, WEB | authenticated mutation에서 필요한 cross-origin method/header를 명시합니다. |
| 3 | `f801ccd09cf0` | `feat(guest): guest runtime 환경 경계 구성` | A | AUTH, WEB | application mode와 explicit proxy-trust switch를 runtime/browser contract에 추가합니다. |
| 4 | `2b274686e6d4` | `fix(guest): 체험 환경의 runtime 복구 제한` | A | AUTH, REALTIME, RISK | `APP_MODE` parsing을 중앙화하고 invalid explicit value를 fail-closed 처리합니다. |
| 5 | `eb675ef74af3` | `fix(config): production에서 영속 저장소 요구` | A | TOURNAMENT, OPERATIONS, RISK | production mode에서 `DATABASE_URL` 누락을 startup 전에 거부합니다. |
| 6 | `4633dfde208d` | `test(config): production memory fallback 거부 검증` | A | OPERATIONS, TEST | explicit/inferred production 모두 memory fallback을 거부하고 demo만 허용하는지 검증합니다. |
...
| Source에서 확정된 역할 | configuration precedence와 local runtime default를 고정합니다. |
...
| Importance | B |
| Tags | AUTH, WEB |
| Source에서 확정된 역할 | authenticated mutation에서 필요한 cross-origin method/header를 명시합니다. |
...
| Importance | A |
| Tags | AUTH, WEB |
| Source에서 확정된 역할 | application mode와 explicit proxy-trust switch를 runtime/browser contract에 추가합니다. |
...
| Source에서 확정된 역할 | `APP_MODE` parsing을 중앙화하고 invalid explicit value를 fail-closed 처리합니다. |
...
| Source에서 확정된 역할 | production mode에서 `DATABASE_URL` 누락을 startup 전에 거부합니다. |
...
| Source에서 확정된 역할 | explicit/inferred production 모두 memory fallback을 거부하고 demo만 허용하는지 검증합니다. |
...
| --- | --- | --- | --- | --- | --- | --- |
| 환경별 capability는 검증된 단일 runtime mode에서 파생됩니다. | [작성] | [작성] | [작성] | [작성] | [작성] | [파일·심볼] |
| production은 영속 저장소 없이 시작하지 않으며 demo의 memory semantics와 혼동되지 않습니다. | [작성] | [작성] | [작성] | [작성] | [작성] | [파일·심볼] |
| client address를 신뢰하는 기능은 명시적으로 proxy trust를 활성화한 경우에만 forwarded header를 사용합니다. | [작성] | [작성] | [작성] | [작성] | [작성] | [파일·심볼] |
...
| --- | --- | --- | --- | --- | --- |
| environment parsing | [작성] | [작성] | [작성] | [작성] | [SHA·파일·심볼] |
| runtime capability matrix | [작성] | [작성] | [작성] | [작성] | [SHA·파일·심볼] |
| CORS policy | [작성] | [작성] | [작성] | [작성] | [SHA·파일·심볼] |
| secure-cookie policy | [작성] | [작성] | [작성] | [작성] | [SHA·파일·심볼] |
| proxy/IP trust | [작성] | [작성] | [작성] | [작성] | [SHA·파일·심볼] |
| repository selection | [작성] | [작성] | [작성] | [작성] | [SHA·파일·심볼] |
...
- Cookie-only credential 전환은 `05-core-realtime-architecture`의 인증 Thread에서 완성됩니다.
