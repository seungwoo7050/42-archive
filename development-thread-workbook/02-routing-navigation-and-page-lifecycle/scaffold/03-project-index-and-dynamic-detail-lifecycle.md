# Thread: Project index and dynamic detail lifecycle
...
`/projects`와 `/projects/[projectId]`가 content aggregate, query state, static params, missing project와 page enablement를 어떻게 소유하는지 복원합니다.

### 계획된 핵심 invariant

- `Project index and dynamic detail lifecycle`의 주요 결정은 route/design/component마다 중복 해석되지 않고 명시된 owner에 위치합니다.
...
| ---: | --- | --- | :---: | --- | --- |
| 1 | `7571f1400065` | feat(projects): 프로젝트 목록 route 연결 | B | ROUTING, RENDERER | 초기 상태와 vocabulary를 고정합니다. |
| 2 | `d4c7f742fb4d` | feat(project): 프로젝트 상세 route 연결 | B | ROUTING, RENDERER | 기능·책임 경계를 확장합니다. |
| 3 | `3e2e95a3a28c` | feat(content): 페이지 활성화 selector 추가 | B | CONTENT, ROUTING | 기능·책임 경계를 확장합니다. |
| 4 | `50199be241c8` | feat(routes): 비활성 페이지 route 차단 | A | ARCH, ROUTING | 기능·책임 경계를 확장합니다. |
| 5 | `154b7e6cb54b` | feat(site): 사용자 정의 404 페이지 추가 | B | - | 기능·책임 경계를 확장합니다. |
| 6 | `850e084b3911` | test(site): 404 복귀 동선 검증 | B | VALIDATION, TEST | Thread의 통합·검증 상태를 확인합니다. |
...
### 4. `50199be241c8` — feat(routes): 비활성 페이지 route 차단
...
### 5. `154b7e6cb54b` — feat(site): 사용자 정의 404 페이지 추가
...
- `154b7e6cb54b^`와 `154b7e6cb54b`의 first-parent diff에서 변경 파일과 핵심 symbol을 확인합니다.
- Resulting tree에서 새 symbol의 caller/callee와 data/state ownership을 추적합니다.
- Commit이 추가한 입력, 출력, optional/disabled/unknown state와 integration point를 확인합니다.
- 이 SHA가 보장하는 범위와 후속 commit에 남긴 미완성 범위를 기록합니다.
...
- 먼저 `154b7e6cb54b^`와 `154b7e6cb54b`를 비교합니다.
- Final HEAD의 helper, test, file layout을 이 commit에 소급하지 않습니다.
- 실행하지 않은 command 결과는 정적 검토와 구분합니다.
...
### 6. `850e084b3911` — test(site): 404 복귀 동선 검증
...
- `850e084b3911^`와 `850e084b3911`의 first-parent diff에서 변경 파일과 핵심 symbol을 확인합니다.
- Resulting tree에서 새 symbol의 caller/callee와 data/state ownership을 추적합니다.
...
- 먼저 `850e084b3911^`와 `850e084b3911`를 비교합니다.
- Final HEAD의 helper, test, file layout을 이 commit에 소급하지 않습니다.
- 실행하지 않은 command 결과는 정적 검토와 구분합니다.
...
| --- | --- | --- | --- | --- |
| `Project index and dynamic detail lifecycle`의 핵심 결정은 한 owner가 수행합니다. |  |  |  |  |
