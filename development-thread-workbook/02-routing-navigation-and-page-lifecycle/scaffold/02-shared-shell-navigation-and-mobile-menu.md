# Thread: Shared shell navigation and mobile menu
...
브랜드, 주 탐색, footer, design switcher와 mobile disclosure를 하나의 route-aware shell contract로 통합하는 과정을 복원합니다.

### 계획된 핵심 invariant

- `Shared shell navigation and mobile menu`의 주요 결정은 route/design/component마다 중복 해석되지 않고 명시된 owner에 위치합니다.
...
| ---: | --- | --- | :---: | --- | --- |
| 1 | `e936c79b98bd` | feat(shell): 브랜드와 주 탐색 헤더 추가 | B | ROUTING | 초기 상태와 vocabulary를 고정합니다. |
| 2 | `a2fd51d4da44` | feat(shell): 홈 디자인 전환 탐색 추가 | B | ROUTING | 기능·책임 경계를 확장합니다. |
| 3 | `8e11443dc26d` | feat(shell): 공용 푸터와 페이지 셸 추가 | A | ARCH, ROUTING | 기능·책임 경계를 확장합니다. |
| 4 | `7f77ec1912d6` | feat(shell): 현재 navigation 상태와 모바일 메뉴 추가 | B | ROUTING | 기능·책임 경계를 확장합니다. |
| 5 | `b9571c485013` | feat(shell): 디자인 선택기를 공용 shell에 연결 | A | ARCH, ROUTING | 기능·책임 경계를 확장합니다. |
| 6 | `51806e1875e7` | style(theme): 디자인 속성을 site shell로 승격 | B | ROUTING, RENDERER | Thread의 통합·검증 상태를 확인합니다. |
...
- `e936c79b98bd^`와 `e936c79b98bd`의 first-parent diff에서 변경 파일과 핵심 symbol을 확인합니다.
...
- 먼저 `e936c79b98bd^`와 `e936c79b98bd`를 비교합니다.
...
- `a2fd51d4da44^`와 `a2fd51d4da44`의 first-parent diff에서 변경 파일과 핵심 symbol을 확인합니다.
- Resulting tree에서 새 symbol의 caller/callee와 data/state ownership을 추적합니다.
- Commit이 추가한 입력, 출력, optional/disabled/unknown state와 integration point를 확인합니다.
- 이 SHA가 보장하는 범위와 후속 commit에 남긴 미완성 범위를 기록합니다.
...
- 먼저 `a2fd51d4da44^`와 `a2fd51d4da44`를 비교합니다.
- Final HEAD의 helper, test, file layout을 이 commit에 소급하지 않습니다.
- 실행하지 않은 command 결과는 정적 검토와 구분합니다.
...
- `8e11443dc26d^`와 `8e11443dc26d`의 first-parent diff에서 변경 파일과 핵심 symbol을 확인합니다.
- Resulting tree에서 새 symbol의 caller/callee와 data/state ownership을 추적합니다.
- Commit이 추가한 입력, 출력, optional/disabled/unknown state와 integration point를 확인합니다.
- 이 SHA가 보장하는 범위와 후속 commit에 남긴 미완성 범위를 기록합니다.
...
- 먼저 `8e11443dc26d^`와 `8e11443dc26d`를 비교합니다.
- Final HEAD의 helper, test, file layout을 이 commit에 소급하지 않습니다.
- 실행하지 않은 command 결과는 정적 검토와 구분합니다.
...
### 4. `7f77ec1912d6` — feat(shell): 현재 navigation 상태와 모바일 메뉴 추가
...
- `7f77ec1912d6^`와 `7f77ec1912d6`의 first-parent diff에서 변경 파일과 핵심 symbol을 확인합니다.
- Resulting tree에서 새 symbol의 caller/callee와 data/state ownership을 추적합니다.
- Commit이 추가한 입력, 출력, optional/disabled/unknown state와 integration point를 확인합니다.
- 이 SHA가 보장하는 범위와 후속 commit에 남긴 미완성 범위를 기록합니다.
...
- 먼저 `7f77ec1912d6^`와 `7f77ec1912d6`를 비교합니다.
- Final HEAD의 helper, test, file layout을 이 commit에 소급하지 않습니다.
- 실행하지 않은 command 결과는 정적 검토와 구분합니다.
...
### 5. `b9571c485013` — feat(shell): 디자인 선택기를 공용 shell에 연결
...
- `b9571c485013^`와 `b9571c485013`의 first-parent diff에서 변경 파일과 핵심 symbol을 확인합니다.
- Resulting tree에서 새 symbol의 caller/callee와 data/state ownership을 추적합니다.
- Commit이 추가한 입력, 출력, optional/disabled/unknown state와 integration point를 확인합니다.
- 이 SHA가 보장하는 범위와 후속 commit에 남긴 미완성 범위를 기록합니다.
...
- 먼저 `b9571c485013^`와 `b9571c485013`를 비교합니다.
- Final HEAD의 helper, test, file layout을 이 commit에 소급하지 않습니다.
- 실행하지 않은 command 결과는 정적 검토와 구분합니다.
...
### 6. `51806e1875e7` — style(theme): 디자인 속성을 site shell로 승격
...
- `51806e1875e7^`와 `51806e1875e7`의 first-parent diff에서 변경 파일과 핵심 symbol을 확인합니다.
- Resulting tree에서 새 symbol의 caller/callee와 data/state ownership을 추적합니다.
...
- 먼저 `51806e1875e7^`와 `51806e1875e7`를 비교합니다.
- Final HEAD의 helper, test, file layout을 이 commit에 소급하지 않습니다.
- 실행하지 않은 command 결과는 정적 검토와 구분합니다.
...
| --- | --- | --- | --- | --- |
| `Shared shell navigation and mobile menu`의 핵심 결정은 한 owner가 수행합니다. |  |  |  |  |
