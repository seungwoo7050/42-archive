# Thread: Page context consolidation
...
각 App Router page에 반복되던 content load, query resolve, active design과 debug state 구성을 하나의 page-context owner로 이동하는 과정을 복원합니다.

### 계획된 핵심 invariant

- `Page context consolidation`의 주요 결정은 route/design/component마다 중복 해석되지 않고 명시된 owner에 위치합니다.
...
| ---: | --- | --- | :---: | --- | --- |
| 1 | `1cf65b708476` | refactor(routes): 홈 page context 통합 | A | ARCH, ROUTING, REFACTOR | 초기 상태와 vocabulary를 고정합니다. |
| 2 | `2075e54ff947` | refactor(projects): 프로젝트 page context 통합 | B | RENDERER, REFACTOR | 기능·책임 경계를 확장합니다. |
| 3 | `a9fdde29221d` | refactor(routes): 소개와 학습 route context 통합 | B | ROUTING, REFACTOR | 기능·책임 경계를 확장합니다. |
| 4 | `349317425bd2` | refactor(routes): 이력과 연락 context 통합 | B | ROUTING, REFACTOR | 기능·책임 경계를 확장합니다. |
| 5 | `42bef4e5783c` | test(routes): 홈과 route presentation 계약 검증 | A | ARCH, VALIDATION, ROUTING | Thread의 통합·검증 상태를 확인합니다. |

## 5. Commit별 학습 기록

각 section은 반드시 해당 SHA의 tree와 parent diff를 기준으로 작성합니다. 같은 commit이 다른 확장 thread에 다시 등장해도 이 thread의 관점에서 별도로 확인합니다.

### 1. `1cf65b708476` — refactor(routes): 홈 page context 통합
...
- `1cf65b708476^`와 `1cf65b708476`의 first-parent diff에서 변경 파일과 핵심 symbol을 확인합니다.
...
- 먼저 `1cf65b708476^`와 `1cf65b708476`를 비교합니다.
...
### 2. `2075e54ff947` — refactor(projects): 프로젝트 page context 통합
...
- `2075e54ff947^`와 `2075e54ff947`의 first-parent diff에서 변경 파일과 핵심 symbol을 확인합니다.
- Resulting tree에서 새 symbol의 caller/callee와 data/state ownership을 추적합니다.
- Refactor 전후 responsibility와 runtime payload/import/call graph 변화를 비교합니다.
- Compatibility branch와 제거 조건, behavior-preservation evidence를 확인합니다.
...
- 먼저 `2075e54ff947^`와 `2075e54ff947`를 비교합니다.
- Final HEAD의 helper, test, file layout을 이 commit에 소급하지 않습니다.
- 실행하지 않은 command 결과는 정적 검토와 구분합니다.
...
### 3. `a9fdde29221d` — refactor(routes): 소개와 학습 route context 통합
...
- `a9fdde29221d^`와 `a9fdde29221d`의 first-parent diff에서 변경 파일과 핵심 symbol을 확인합니다.
- Resulting tree에서 새 symbol의 caller/callee와 data/state ownership을 추적합니다.
- Refactor 전후 responsibility와 runtime payload/import/call graph 변화를 비교합니다.
- Compatibility branch와 제거 조건, behavior-preservation evidence를 확인합니다.
...
- 먼저 `a9fdde29221d^`와 `a9fdde29221d`를 비교합니다.
- Final HEAD의 helper, test, file layout을 이 commit에 소급하지 않습니다.
- 실행하지 않은 command 결과는 정적 검토와 구분합니다.
...
### 4. `349317425bd2` — refactor(routes): 이력과 연락 context 통합
...
- `349317425bd2^`와 `349317425bd2`의 first-parent diff에서 변경 파일과 핵심 symbol을 확인합니다.
- Resulting tree에서 새 symbol의 caller/callee와 data/state ownership을 추적합니다.
- Refactor 전후 responsibility와 runtime payload/import/call graph 변화를 비교합니다.
- Compatibility branch와 제거 조건, behavior-preservation evidence를 확인합니다.
...
- 먼저 `349317425bd2^`와 `349317425bd2`를 비교합니다.
- Final HEAD의 helper, test, file layout을 이 commit에 소급하지 않습니다.
- 실행하지 않은 command 결과는 정적 검토와 구분합니다.
...
### 5. `42bef4e5783c` — test(routes): 홈과 route presentation 계약 검증
...
| --- | --- | --- | --- | --- |
| `Page context consolidation`의 핵심 결정은 한 owner가 수행합니다. |  |  |  |  |
