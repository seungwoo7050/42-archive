# Thread: Query state and route-preserving navigation
...
선택된 design과 debug state를 URL에 보존하고 invalid query와 protocol-relative path를 안전하게 처리하는 canonical navigation contract를 복원합니다.

### 계획된 핵심 invariant

- `Query state and route-preserving navigation`의 주요 결정은 route/design/component마다 중복 해석되지 않고 명시된 owner에 위치합니다.
...
| 3 | `f4233f024890` | feat(home): 쿼리 기반 디자인 전환 연결 | A | ARCH, ROUTING, RENDERER | 기능·책임 경계를 확장합니다. |
| 4 | `b669e04c0932` | refactor(navigation): 디자인 전환 URL 기본값 명시 | A | ARCH, ROUTING, REFACTOR | Thread의 통합·검증 상태를 확인합니다. |
...
### 3. `f4233f024890` — feat(home): 쿼리 기반 디자인 전환 연결
...
- `f4233f024890^`와 `f4233f024890`의 first-parent diff에서 변경 파일과 핵심 symbol을 확인합니다.
- Resulting tree에서 새 symbol의 caller/callee와 data/state ownership을 추적합니다.
- Commit이 추가한 입력, 출력, optional/disabled/unknown state와 integration point를 확인합니다.
- 이 SHA가 보장하는 범위와 후속 commit에 남긴 미완성 범위를 기록합니다.
...
- 먼저 `f4233f024890^`와 `f4233f024890`를 비교합니다.
- Final HEAD의 helper, test, file layout을 이 commit에 소급하지 않습니다.
- 실행하지 않은 command 결과는 정적 검토와 구분합니다.
...
- `b669e04c0932^`와 `b669e04c0932`의 first-parent diff에서 변경 파일과 핵심 symbol을 확인합니다.
- Resulting tree에서 새 symbol의 caller/callee와 data/state ownership을 추적합니다.
...
- 먼저 `b669e04c0932^`와 `b669e04c0932`를 비교합니다.
- Final HEAD의 helper, test, file layout을 이 commit에 소급하지 않습니다.
- 실행하지 않은 command 결과는 정적 검토와 구분합니다.
...
| --- | --- | --- | --- | --- |
| `Query state and route-preserving navigation`의 핵심 결정은 한 owner가 수행합니다. |  |  |  |  |
