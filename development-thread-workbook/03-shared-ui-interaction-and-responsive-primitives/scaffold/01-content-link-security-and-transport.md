# Thread: Content link security and transport
...
Typed link가 internal Next navigation, external anchor, deployment visibility와 placement policy로 나뉘는 공용 transport boundary를 복원합니다.

### 계획된 핵심 invariant

- `Content link security and transport`의 주요 결정은 route/design/component마다 중복 해석되지 않고 명시된 owner에 위치합니다.
...
| ---: | --- | --- | :---: | --- | --- |
| 1 | `ba8da56d3fcf` | feat(portfolio): 연락과 프로젝트 링크 선택기 추가 | A | CONTENT | 초기 상태와 vocabulary를 고정합니다. |
| 2 | `f63c978c71c9` | feat(ui): 내부 외부 콘텐츠 링크 렌더링 | A | CONTENT | 기능·책임 경계를 확장합니다. |
| 3 | `e37ea9c2819a` | feat(project): 프로젝트 링크 그룹 추가 | B | RENDERER | 기능·책임 경계를 확장합니다. |
| 4 | `1ef269fbdb49` | feat(project): 프로젝트 카드 링크 추가 | B | RENDERER | 기능·책임 경계를 확장합니다. |
| 5 | `daa6815a6dfa` | feat(project): 카드 링크를 콘텐츠 배치 기준으로 선택 | B | CONTENT, RENDERER | 기능·책임 경계를 확장합니다. |
| 6 | `44e4d062da50` | refactor(ui): 프로젝트 링크 렌더링 중복 제거 | B | VALIDATION, REFACTOR | Thread의 통합·검증 상태를 확인합니다. |
...
### 6. `44e4d062da50` — refactor(ui): 프로젝트 링크 렌더링 중복 제거
...
- `44e4d062da50^`와 `44e4d062da50`의 first-parent diff에서 변경 파일과 핵심 symbol을 확인합니다.
- Resulting tree에서 새 symbol의 caller/callee와 data/state ownership을 추적합니다.
...
- 먼저 `44e4d062da50^`와 `44e4d062da50`를 비교합니다.
- Final HEAD의 helper, test, file layout을 이 commit에 소급하지 않습니다.
- 실행하지 않은 command 결과는 정적 검토와 구분합니다.
...
| --- | --- | --- | --- | --- |
| `Content link security and transport`의 핵심 결정은 한 owner가 수행합니다. |  |  |  |  |
