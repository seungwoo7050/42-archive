# Thread: Project card actions and evidence components
...
Deployment badge, screenshot, stack, highlight, provenance와 action selector가 재사용 가능한 project-card boundary로 결합되는 과정을 복원합니다.

### 계획된 핵심 invariant

- `Project card actions and evidence components`의 주요 결정은 route/design/component마다 중복 해석되지 않고 명시된 owner에 위치합니다.
...
| ---: | --- | --- | :---: | --- | --- |
| 1 | `53ca0860ef3a` | feat(project): 프로젝트 배포 상태 배지 추가 | B | RENDERER | 초기 상태와 vocabulary를 고정합니다. |
| 2 | `e37ea9c2819a` | feat(project): 프로젝트 링크 그룹 추가 | B | RENDERER | 기능·책임 경계를 확장합니다. |
| 3 | `1ef269fbdb49` | feat(project): 프로젝트 카드 링크 추가 | B | RENDERER | 기능·책임 경계를 확장합니다. |
| 4 | `b72c52a22690` | feat(project): 프로젝트 카드 프리미티브 추가 | B | RENDERER | 기능·책임 경계를 확장합니다. |
| 5 | `4000a8657a62` | style(project): 프로젝트 카드 상호작용 추가 | B | RENDERER | 기능·책임 경계를 확장합니다. |
| 6 | `daa6815a6dfa` | feat(project): 카드 링크를 콘텐츠 배치 기준으로 선택 | B | CONTENT, RENDERER | Thread의 통합·검증 상태를 확인합니다. |
...
- `53ca0860ef3a^`와 `53ca0860ef3a`의 first-parent diff에서 변경 파일과 핵심 symbol을 확인합니다.
...
- 먼저 `53ca0860ef3a^`와 `53ca0860ef3a`를 비교합니다.
...
### 2. `e37ea9c2819a` — feat(project): 프로젝트 링크 그룹 추가
...
- `e37ea9c2819a^`와 `e37ea9c2819a`의 first-parent diff에서 변경 파일과 핵심 symbol을 확인합니다.
- Resulting tree에서 새 symbol의 caller/callee와 data/state ownership을 추적합니다.
- Commit이 추가한 입력, 출력, optional/disabled/unknown state와 integration point를 확인합니다.
- 이 SHA가 보장하는 범위와 후속 commit에 남긴 미완성 범위를 기록합니다.
...
- 먼저 `e37ea9c2819a^`와 `e37ea9c2819a`를 비교합니다.
- Final HEAD의 helper, test, file layout을 이 commit에 소급하지 않습니다.
- 실행하지 않은 command 결과는 정적 검토와 구분합니다.
...
### 3. `1ef269fbdb49` — feat(project): 프로젝트 카드 링크 추가
...
- `1ef269fbdb49^`와 `1ef269fbdb49`의 first-parent diff에서 변경 파일과 핵심 symbol을 확인합니다.
- Resulting tree에서 새 symbol의 caller/callee와 data/state ownership을 추적합니다.
- Commit이 추가한 입력, 출력, optional/disabled/unknown state와 integration point를 확인합니다.
- 이 SHA가 보장하는 범위와 후속 commit에 남긴 미완성 범위를 기록합니다.
...
- 먼저 `1ef269fbdb49^`와 `1ef269fbdb49`를 비교합니다.
- Final HEAD의 helper, test, file layout을 이 commit에 소급하지 않습니다.
- 실행하지 않은 command 결과는 정적 검토와 구분합니다.
...
### 4. `b72c52a22690` — feat(project): 프로젝트 카드 프리미티브 추가
...
- `b72c52a22690^`와 `b72c52a22690`의 first-parent diff에서 변경 파일과 핵심 symbol을 확인합니다.
- Resulting tree에서 새 symbol의 caller/callee와 data/state ownership을 추적합니다.
- Commit이 추가한 입력, 출력, optional/disabled/unknown state와 integration point를 확인합니다.
- 이 SHA가 보장하는 범위와 후속 commit에 남긴 미완성 범위를 기록합니다.
...
- 먼저 `b72c52a22690^`와 `b72c52a22690`를 비교합니다.
- Final HEAD의 helper, test, file layout을 이 commit에 소급하지 않습니다.
- 실행하지 않은 command 결과는 정적 검토와 구분합니다.
...
### 5. `4000a8657a62` — style(project): 프로젝트 카드 상호작용 추가
...
- `4000a8657a62^`와 `4000a8657a62`의 first-parent diff에서 변경 파일과 핵심 symbol을 확인합니다.
- Resulting tree에서 새 symbol의 caller/callee와 data/state ownership을 추적합니다.
...
- 먼저 `4000a8657a62^`와 `4000a8657a62`를 비교합니다.
- Final HEAD의 helper, test, file layout을 이 commit에 소급하지 않습니다.
- 실행하지 않은 command 결과는 정적 검토와 구분합니다.
...
### 6. `daa6815a6dfa` — feat(project): 카드 링크를 콘텐츠 배치 기준으로 선택
...
- `daa6815a6dfa^`와 `daa6815a6dfa`의 first-parent diff에서 변경 파일과 핵심 symbol을 확인합니다.
- Resulting tree에서 새 symbol의 caller/callee와 data/state ownership을 추적합니다.
- Commit이 추가한 입력, 출력, optional/disabled/unknown state와 integration point를 확인합니다.
- 이 SHA가 보장하는 범위와 후속 commit에 남긴 미완성 범위를 기록합니다.
...
- 먼저 `daa6815a6dfa^`와 `daa6815a6dfa`를 비교합니다.
- Final HEAD의 helper, test, file layout을 이 commit에 소급하지 않습니다.
- 실행하지 않은 command 결과는 정적 검토와 구분합니다.
...
| --- | --- | --- | --- | --- |
| `Project card actions and evidence components`의 핵심 결정은 한 owner가 수행합니다. |  |  |  |  |
