...
Profile photo와 project screenshot primitive가 priority, lazy loading, aspect ratio, empty state와 debug provenance를 어떻게 표준화하는지 복원합니다.

### 계획된 핵심 invariant

- `Content media loading and layout stability`의 주요 결정은 route/design/component마다 중복 해석되지 않고 명시된 owner에 위치합니다.
...
| ---: | --- | --- | :---: | --- | --- |
| 1 | `aa115c73ae30` | feat(ui): 콘텐츠 이미지 프리미티브 추가 | B | CONTENT | 초기 상태와 vocabulary를 고정합니다. |
| 2 | `b027f42669aa` | feat(home): 대표 프로젝트 쇼케이스 추가 | B | RENDERER | 기능·책임 경계를 확장합니다. |
| 3 | `06fff9a6e93b` | feat(project): 프로젝트 상세 소개 추가 | B | RENDERER | 기능·책임 경계를 확장합니다. |
| 4 | `cabf3a0e378f` | feat(project): 프로젝트 구조와 증거 갤러리 추가 | B | RENDERER | Thread의 통합·검증 상태를 확인합니다. |
...
- `aa115c73ae30^`와 `aa115c73ae30`의 first-parent diff에서 변경 파일과 핵심 symbol을 확인합니다.
...
- 먼저 `aa115c73ae30^`와 `aa115c73ae30`를 비교합니다.
...
| --- | --- | --- | --- | --- |
| `Content media loading and layout stability`의 핵심 결정은 한 owner가 수행합니다. |  |  |  |  |
