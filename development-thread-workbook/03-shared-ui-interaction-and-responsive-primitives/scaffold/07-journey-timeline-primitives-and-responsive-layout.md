...
단일 카드/date formatter에서 compact·paired-centerline variant, desktop geometry와 mobile reflow까지 timeline primitive를 발전시키는 과정을 복원합니다.

### 계획된 핵심 invariant

- `Journey timeline primitives and responsive layout`의 주요 결정은 route/design/component마다 중복 해석되지 않고 명시된 owner에 위치합니다.
...
| ---: | --- | --- | :---: | --- | --- |
| 1 | `f60edf14dae0` | feat(journey): 여정 날짜와 카드 프리미티브 추가 | B | RENDERER | 초기 상태와 vocabulary를 고정합니다. |
| 2 | `3c0a1154ba08` | feat(journey): 중앙선 여정 목록 추가 | B | RENDERER | 기능·책임 경계를 확장합니다. |
| 3 | `b3182e35aed0` | feat(journey): 여정 목록 변형 연결 | B | RENDERER | 기능·책임 경계를 확장합니다. |
| 4 | `377fa128f82b` | style(journey): 여정 타임라인 시각 계층 추가 | B | RENDERER | 기능·책임 경계를 확장합니다. |
| 5 | `7995a3cf5435` | style(journey): 데스크톱 중앙선 여정 구성 | B | RENDERER | 기능·책임 경계를 확장합니다. |
| 6 | `6394188022fd` | style(journey): 모바일 중앙선 여정 구성 | B | RENDERER | Thread의 통합·검증 상태를 확인합니다. |
...
- `f60edf14dae0^`와 `f60edf14dae0`의 first-parent diff에서 변경 파일과 핵심 symbol을 확인합니다.
...
- 먼저 `f60edf14dae0^`와 `f60edf14dae0`를 비교합니다.
...
- `3c0a1154ba08^`와 `3c0a1154ba08`의 first-parent diff에서 변경 파일과 핵심 symbol을 확인합니다.
- Resulting tree에서 새 symbol의 caller/callee와 data/state ownership을 추적합니다.
- Commit이 추가한 입력, 출력, optional/disabled/unknown state와 integration point를 확인합니다.
- 이 SHA가 보장하는 범위와 후속 commit에 남긴 미완성 범위를 기록합니다.
...
- 먼저 `3c0a1154ba08^`와 `3c0a1154ba08`를 비교합니다.
- Final HEAD의 helper, test, file layout을 이 commit에 소급하지 않습니다.
- 실행하지 않은 command 결과는 정적 검토와 구분합니다.
...
- `b3182e35aed0^`와 `b3182e35aed0`의 first-parent diff에서 변경 파일과 핵심 symbol을 확인합니다.
- Resulting tree에서 새 symbol의 caller/callee와 data/state ownership을 추적합니다.
- Commit이 추가한 입력, 출력, optional/disabled/unknown state와 integration point를 확인합니다.
- 이 SHA가 보장하는 범위와 후속 commit에 남긴 미완성 범위를 기록합니다.
...
- 먼저 `b3182e35aed0^`와 `b3182e35aed0`를 비교합니다.
- Final HEAD의 helper, test, file layout을 이 commit에 소급하지 않습니다.
- 실행하지 않은 command 결과는 정적 검토와 구분합니다.
...
- `377fa128f82b^`와 `377fa128f82b`의 first-parent diff에서 변경 파일과 핵심 symbol을 확인합니다.
- Resulting tree에서 새 symbol의 caller/callee와 data/state ownership을 추적합니다.
...
- 먼저 `377fa128f82b^`와 `377fa128f82b`를 비교합니다.
- Final HEAD의 helper, test, file layout을 이 commit에 소급하지 않습니다.
- 실행하지 않은 command 결과는 정적 검토와 구분합니다.
...
- `7995a3cf5435^`와 `7995a3cf5435`의 first-parent diff에서 변경 파일과 핵심 symbol을 확인합니다.
- Resulting tree에서 새 symbol의 caller/callee와 data/state ownership을 추적합니다.
- 적용 selector, token, breakpoint, motion/focus/semantics 영향과 DOM contract를 확인합니다.
- Visual change가 interaction 또는 accessibility invariant를 바꾸는지 구분합니다.
...
- 먼저 `7995a3cf5435^`와 `7995a3cf5435`를 비교합니다.
- Final HEAD의 helper, test, file layout을 이 commit에 소급하지 않습니다.
- 실행하지 않은 command 결과는 정적 검토와 구분합니다.
...
- `6394188022fd^`와 `6394188022fd`의 first-parent diff에서 변경 파일과 핵심 symbol을 확인합니다.
- Resulting tree에서 새 symbol의 caller/callee와 data/state ownership을 추적합니다.
- 적용 selector, token, breakpoint, motion/focus/semantics 영향과 DOM contract를 확인합니다.
- Visual change가 interaction 또는 accessibility invariant를 바꾸는지 구분합니다.
...
- 먼저 `6394188022fd^`와 `6394188022fd`를 비교합니다.
- Final HEAD의 helper, test, file layout을 이 commit에 소급하지 않습니다.
- 실행하지 않은 command 결과는 정적 검토와 구분합니다.
...
| --- | --- | --- | --- | --- |
| `Journey timeline primitives and responsive layout`의 핵심 결정은 한 owner가 수행합니다. |  |  |  |  |
