...
다섯 design이 독립적인 visual language를 유지하면서도 공통 route contract, shell state, token roles와 browser/visual regression surface를 공유하는 과정을 복원합니다.

### 계획된 핵심 invariant

- `Cross-design token, shell, and regression contracts`의 주요 결정은 route/design/component마다 중복 해석되지 않고 명시된 owner에 위치합니다.
...
| ---: | --- | --- | :---: | --- | --- |
| 1 | `418e7bc1d8bb` | feat(designs): site design 정의 registry 추가 | A | ARCH, RENDERER | 초기 상태와 vocabulary를 고정합니다. |
| 2 | `e14202198948` | feat(designs): route renderer 계약 추가 | A | ARCH, ROUTING, RENDERER | 기능·책임 경계를 확장합니다. |
| 3 | `6fc28f4c6586` | refactor(designs): 확장 renderer lazy registry 추가 | A | ARCH, RENDERER, REFACTOR | 기능·책임 경계를 확장합니다. |
| 4 | `4732301a7d2c` | style(designs): route renderer 디자인 토큰 확장 | A | ARCH, ROUTING, RENDERER | 기능·책임 경계를 확장합니다. |
| 5 | `969741c3469d` | refactor(shell): 디자인 renderer 셸 경계 추가 | A | ARCH, ROUTING, RENDERER | 기능·책임 경계를 확장합니다. |
| 6 | `1598a87702f6` | test(design): view model 기반 renderer matrix 검증 | A | ARCH, VALIDATION, RENDERER | 기능·책임 경계를 확장합니다. |
| 7 | `055b733cbb7e` | test(design): 독립 renderer와 design token 경계 검증 | A | ARCH, VALIDATION, RENDERER | 기능·책임 경계를 확장합니다. |
| 8 | `882a2f9d753e` | test(visual): 다섯 디자인 회귀 기준 추가 | A | TEST | Thread의 통합·검증 상태를 확인합니다. |
...
- `418e7bc1d8bb^`와 `418e7bc1d8bb`의 first-parent diff에서 변경 파일과 핵심 symbol을 확인합니다.
...
- 먼저 `418e7bc1d8bb^`와 `418e7bc1d8bb`를 비교합니다.
...
- `e14202198948^`와 `e14202198948`의 first-parent diff에서 변경 파일과 핵심 symbol을 확인합니다.
- Resulting tree에서 새 symbol의 caller/callee와 data/state ownership을 추적합니다.
- Commit이 추가한 입력, 출력, optional/disabled/unknown state와 integration point를 확인합니다.
- 이 SHA가 보장하는 범위와 후속 commit에 남긴 미완성 범위를 기록합니다.
...
- 먼저 `e14202198948^`와 `e14202198948`를 비교합니다.
- Final HEAD의 helper, test, file layout을 이 commit에 소급하지 않습니다.
- 실행하지 않은 command 결과는 정적 검토와 구분합니다.
...
- `6fc28f4c6586^`와 `6fc28f4c6586`의 first-parent diff에서 변경 파일과 핵심 symbol을 확인합니다.
- Resulting tree에서 새 symbol의 caller/callee와 data/state ownership을 추적합니다.
...
- 먼저 `6fc28f4c6586^`와 `6fc28f4c6586`를 비교합니다.
- Final HEAD의 helper, test, file layout을 이 commit에 소급하지 않습니다.
- 실행하지 않은 command 결과는 정적 검토와 구분합니다.
...
### 4. `4732301a7d2c` — style(designs): route renderer 디자인 토큰 확장
...
### 5. `969741c3469d` — refactor(shell): 디자인 renderer 셸 경계 추가
...
### 6. `1598a87702f6` — test(design): view model 기반 renderer matrix 검증
...
### 7. `055b733cbb7e` — test(design): 독립 renderer와 design token 경계 검증
...
### 8. `882a2f9d753e` — test(visual): 다섯 디자인 회귀 기준 추가
...
- `882a2f9d753e^`와 `882a2f9d753e`의 first-parent diff에서 변경 파일과 핵심 symbol을 확인합니다.
- Resulting tree에서 새 symbol의 caller/callee와 data/state ownership을 추적합니다.
- 대상 production invariant, fixture/failure injection, test technique와 실제 production path를 구분합니다.
- Test가 증명하는 것과 증명하지 않는 것을 명시합니다.
...
- 먼저 `882a2f9d753e^`와 `882a2f9d753e`를 비교합니다.
- Final HEAD의 helper, test, file layout을 이 commit에 소급하지 않습니다.
- 실행하지 않은 command 결과는 정적 검토와 구분합니다.
...
| --- | --- | --- | --- | --- |
| `Cross-design token, shell, and regression contracts`의 핵심 결정은 한 owner가 수행합니다. |  |  |  |  |
