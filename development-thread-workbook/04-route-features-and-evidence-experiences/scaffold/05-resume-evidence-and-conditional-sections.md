# Thread: Résumé evidence and conditional sections
...
Résumé route가 profile identity, download, summary, selected projects, experience, training, education와 notes를 structured content에서 조건부로 조립하는 과정을 복원합니다.

### 계획된 핵심 invariant

- `Résumé evidence and conditional sections`의 주요 결정은 route/design/component마다 중복 해석되지 않고 명시된 owner에 위치합니다.
...
| ---: | --- | --- | :---: | --- | --- |
| 1 | `e655951b0706` | feat(resume): 이력 소개와 요약 추가 | B | RENDERER | 초기 상태와 vocabulary를 고정합니다. |
| 2 | `b399ce0c7f84` | feat(resume): 선택 프로젝트 경력 추가 | B | RENDERER | 기능·책임 경계를 확장합니다. |
| 3 | `4d17fd7ab81b` | feat(resume): 교육 과정 요약 추가 | B | RENDERER | 기능·책임 경계를 확장합니다. |
| 4 | `f64763cffcff` | feat(resume): 프로필 위치와 가용성 추가 | B | RENDERER | 기능·책임 경계를 확장합니다. |
| 5 | `3c113f1995ff` | feat(resume): 경력 이력 추가 | B | RENDERER | 기능·책임 경계를 확장합니다. |
| 6 | `732ae5a6785c` | feat(resume): 교육 이력 추가 | B | RENDERER | 기능·책임 경계를 확장합니다. |
| 7 | `579ea168daa8` | feat(resume): Resume 안내 기록 추가 | B | RENDERER | Thread의 통합·검증 상태를 확인합니다. |

## 5. Commit별 학습 기록

각 section은 반드시 해당 SHA의 tree와 parent diff를 기준으로 작성합니다. 같은 commit이 다른 확장 thread에 다시 등장해도 이 thread의 관점에서 별도로 확인합니다.

### 1. `e655951b0706` — feat(resume): 이력 소개와 요약 추가
...
- `e655951b0706^`와 `e655951b0706`의 first-parent diff에서 변경 파일과 핵심 symbol을 확인합니다.
...
- 먼저 `e655951b0706^`와 `e655951b0706`를 비교합니다.
...
### 2. `b399ce0c7f84` — feat(resume): 선택 프로젝트 경력 추가
...
- `b399ce0c7f84^`와 `b399ce0c7f84`의 first-parent diff에서 변경 파일과 핵심 symbol을 확인합니다.
- Resulting tree에서 새 symbol의 caller/callee와 data/state ownership을 추적합니다.
- Commit이 추가한 입력, 출력, optional/disabled/unknown state와 integration point를 확인합니다.
- 이 SHA가 보장하는 범위와 후속 commit에 남긴 미완성 범위를 기록합니다.
...
- 먼저 `b399ce0c7f84^`와 `b399ce0c7f84`를 비교합니다.
- Final HEAD의 helper, test, file layout을 이 commit에 소급하지 않습니다.
- 실행하지 않은 command 결과는 정적 검토와 구분합니다.
...
### 3. `4d17fd7ab81b` — feat(resume): 교육 과정 요약 추가
...
- `4d17fd7ab81b^`와 `4d17fd7ab81b`의 first-parent diff에서 변경 파일과 핵심 symbol을 확인합니다.
- Resulting tree에서 새 symbol의 caller/callee와 data/state ownership을 추적합니다.
- Commit이 추가한 입력, 출력, optional/disabled/unknown state와 integration point를 확인합니다.
- 이 SHA가 보장하는 범위와 후속 commit에 남긴 미완성 범위를 기록합니다.
...
- 먼저 `4d17fd7ab81b^`와 `4d17fd7ab81b`를 비교합니다.
- Final HEAD의 helper, test, file layout을 이 commit에 소급하지 않습니다.
- 실행하지 않은 command 결과는 정적 검토와 구분합니다.
...
### 4. `f64763cffcff` — feat(resume): 프로필 위치와 가용성 추가
...
- `f64763cffcff^`와 `f64763cffcff`의 first-parent diff에서 변경 파일과 핵심 symbol을 확인합니다.
- Resulting tree에서 새 symbol의 caller/callee와 data/state ownership을 추적합니다.
- Commit이 추가한 입력, 출력, optional/disabled/unknown state와 integration point를 확인합니다.
- 이 SHA가 보장하는 범위와 후속 commit에 남긴 미완성 범위를 기록합니다.
...
- 먼저 `f64763cffcff^`와 `f64763cffcff`를 비교합니다.
- Final HEAD의 helper, test, file layout을 이 commit에 소급하지 않습니다.
- 실행하지 않은 command 결과는 정적 검토와 구분합니다.
...
### 5. `3c113f1995ff` — feat(resume): 경력 이력 추가
...
- `3c113f1995ff^`와 `3c113f1995ff`의 first-parent diff에서 변경 파일과 핵심 symbol을 확인합니다.
- Resulting tree에서 새 symbol의 caller/callee와 data/state ownership을 추적합니다.
- Commit이 추가한 입력, 출력, optional/disabled/unknown state와 integration point를 확인합니다.
- 이 SHA가 보장하는 범위와 후속 commit에 남긴 미완성 범위를 기록합니다.
...
- 먼저 `3c113f1995ff^`와 `3c113f1995ff`를 비교합니다.
- Final HEAD의 helper, test, file layout을 이 commit에 소급하지 않습니다.
- 실행하지 않은 command 결과는 정적 검토와 구분합니다.
...
### 6. `732ae5a6785c` — feat(resume): 교육 이력 추가
...
- `732ae5a6785c^`와 `732ae5a6785c`의 first-parent diff에서 변경 파일과 핵심 symbol을 확인합니다.
- Resulting tree에서 새 symbol의 caller/callee와 data/state ownership을 추적합니다.
- Commit이 추가한 입력, 출력, optional/disabled/unknown state와 integration point를 확인합니다.
- 이 SHA가 보장하는 범위와 후속 commit에 남긴 미완성 범위를 기록합니다.
...
- 먼저 `732ae5a6785c^`와 `732ae5a6785c`를 비교합니다.
- Final HEAD의 helper, test, file layout을 이 commit에 소급하지 않습니다.
- 실행하지 않은 command 결과는 정적 검토와 구분합니다.
...
### 7. `579ea168daa8` — feat(resume): Resume 안내 기록 추가
...
- `579ea168daa8^`와 `579ea168daa8`의 first-parent diff에서 변경 파일과 핵심 symbol을 확인합니다.
- Resulting tree에서 새 symbol의 caller/callee와 data/state ownership을 추적합니다.
- Commit이 추가한 입력, 출력, optional/disabled/unknown state와 integration point를 확인합니다.
- 이 SHA가 보장하는 범위와 후속 commit에 남긴 미완성 범위를 기록합니다.
...
- 먼저 `579ea168daa8^`와 `579ea168daa8`를 비교합니다.
- Final HEAD의 helper, test, file layout을 이 commit에 소급하지 않습니다.
- 실행하지 않은 command 결과는 정적 검토와 구분합니다.
...
| --- | --- | --- | --- | --- |
| `Résumé evidence and conditional sections`의 핵심 결정은 한 owner가 수행합니다. |  |  |  |  |
