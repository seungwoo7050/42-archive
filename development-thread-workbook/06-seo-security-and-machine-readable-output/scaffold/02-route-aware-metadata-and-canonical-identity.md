...
Validated content에서 site/route별 title, canonical, Open Graph와 Twitter metadata를 만들고 모든 public route export에 연결하는 과정을 복원합니다.

### 계획된 핵심 invariant

- `Route-aware metadata and canonical identity`의 주요 결정은 route/design/component마다 중복 해석되지 않고 명시된 owner에 위치합니다.
...
| ---: | --- | --- | :---: | --- | --- |
| 1 | `1f4f93ad9a0f` | feat(metadata): 콘텐츠 기반 site metadata 추가 | A | CONTENT, SEO | 초기 상태와 vocabulary를 고정합니다. |
| 2 | `1c40645caead` | feat(seo): route별 검색 metadata 정책 추가 | A | ARCH, ROUTING, SEO | 기능·책임 경계를 확장합니다. |
| 3 | `844ff4d7abcb` | feat(seo): 홈과 프로젝트 route metadata 연결 | B | ROUTING, SEO | 기능·책임 경계를 확장합니다. |
| 4 | `fd5ff532bfe9` | feat(seo): 프로필 route metadata 연결 | B | ROUTING, SEO | 기능·책임 경계를 확장합니다. |
| 5 | `5632c5df9b47` | feat(seo): 여정과 근거 route metadata 연결 | B | ROUTING, SEO | 기능·책임 경계를 확장합니다. |
| 6 | `4358bcd34f2e` | test(seo): route metadata export 검증 | B | VALIDATION, ROUTING, SEO | 기능·책임 경계를 확장합니다. |
| 7 | `adc392157f70` | test(seo): route metadata와 sitemap 계약 검증 | B | VALIDATION, ROUTING, SEO | Thread의 통합·검증 상태를 확인합니다. |

## 5. Commit별 학습 기록

각 section은 반드시 해당 SHA의 tree와 parent diff를 기준으로 작성합니다. 같은 commit이 다른 확장 thread에 다시 등장해도 이 thread의 관점에서 별도로 확인합니다.

### 1. `1f4f93ad9a0f` — feat(metadata): 콘텐츠 기반 site metadata 추가
...
- `1f4f93ad9a0f^`와 `1f4f93ad9a0f`의 first-parent diff에서 변경 파일과 핵심 symbol을 확인합니다.
...
- 먼저 `1f4f93ad9a0f^`와 `1f4f93ad9a0f`를 비교합니다.
...
### 2. `1c40645caead` — feat(seo): route별 검색 metadata 정책 추가
...
- `1c40645caead^`와 `1c40645caead`의 first-parent diff에서 변경 파일과 핵심 symbol을 확인합니다.
- Resulting tree에서 새 symbol의 caller/callee와 data/state ownership을 추적합니다.
- Commit이 추가한 입력, 출력, optional/disabled/unknown state와 integration point를 확인합니다.
- 이 SHA가 보장하는 범위와 후속 commit에 남긴 미완성 범위를 기록합니다.
...
- 먼저 `1c40645caead^`와 `1c40645caead`를 비교합니다.
- Final HEAD의 helper, test, file layout을 이 commit에 소급하지 않습니다.
- 실행하지 않은 command 결과는 정적 검토와 구분합니다.
...
### 3. `844ff4d7abcb` — feat(seo): 홈과 프로젝트 route metadata 연결
...
- `844ff4d7abcb^`와 `844ff4d7abcb`의 first-parent diff에서 변경 파일과 핵심 symbol을 확인합니다.
- Resulting tree에서 새 symbol의 caller/callee와 data/state ownership을 추적합니다.
- Commit이 추가한 입력, 출력, optional/disabled/unknown state와 integration point를 확인합니다.
- 이 SHA가 보장하는 범위와 후속 commit에 남긴 미완성 범위를 기록합니다.
...
- 먼저 `844ff4d7abcb^`와 `844ff4d7abcb`를 비교합니다.
- Final HEAD의 helper, test, file layout을 이 commit에 소급하지 않습니다.
- 실행하지 않은 command 결과는 정적 검토와 구분합니다.
...
### 4. `fd5ff532bfe9` — feat(seo): 프로필 route metadata 연결
...
- `fd5ff532bfe9^`와 `fd5ff532bfe9`의 first-parent diff에서 변경 파일과 핵심 symbol을 확인합니다.
- Resulting tree에서 새 symbol의 caller/callee와 data/state ownership을 추적합니다.
- Commit이 추가한 입력, 출력, optional/disabled/unknown state와 integration point를 확인합니다.
- 이 SHA가 보장하는 범위와 후속 commit에 남긴 미완성 범위를 기록합니다.
...
- 먼저 `fd5ff532bfe9^`와 `fd5ff532bfe9`를 비교합니다.
- Final HEAD의 helper, test, file layout을 이 commit에 소급하지 않습니다.
- 실행하지 않은 command 결과는 정적 검토와 구분합니다.
...
### 5. `5632c5df9b47` — feat(seo): 여정과 근거 route metadata 연결
...
- `5632c5df9b47^`와 `5632c5df9b47`의 first-parent diff에서 변경 파일과 핵심 symbol을 확인합니다.
- Resulting tree에서 새 symbol의 caller/callee와 data/state ownership을 추적합니다.
- Commit이 추가한 입력, 출력, optional/disabled/unknown state와 integration point를 확인합니다.
- 이 SHA가 보장하는 범위와 후속 commit에 남긴 미완성 범위를 기록합니다.
...
- 먼저 `5632c5df9b47^`와 `5632c5df9b47`를 비교합니다.
- Final HEAD의 helper, test, file layout을 이 commit에 소급하지 않습니다.
- 실행하지 않은 command 결과는 정적 검토와 구분합니다.
...
### 6. `4358bcd34f2e` — test(seo): route metadata export 검증
...
- `4358bcd34f2e^`와 `4358bcd34f2e`의 first-parent diff에서 변경 파일과 핵심 symbol을 확인합니다.
- Resulting tree에서 새 symbol의 caller/callee와 data/state ownership을 추적합니다.
...
- 먼저 `4358bcd34f2e^`와 `4358bcd34f2e`를 비교합니다.
- Final HEAD의 helper, test, file layout을 이 commit에 소급하지 않습니다.
- 실행하지 않은 command 결과는 정적 검토와 구분합니다.
...
### 7. `adc392157f70` — test(seo): route metadata와 sitemap 계약 검증
...
- `adc392157f70^`와 `adc392157f70`의 first-parent diff에서 변경 파일과 핵심 symbol을 확인합니다.
- Resulting tree에서 새 symbol의 caller/callee와 data/state ownership을 추적합니다.
- 대상 production invariant, fixture/failure injection, test technique와 실제 production path를 구분합니다.
- Test가 증명하는 것과 증명하지 않는 것을 명시합니다.
...
- 먼저 `adc392157f70^`와 `adc392157f70`를 비교합니다.
- Final HEAD의 helper, test, file layout을 이 commit에 소급하지 않습니다.
- 실행하지 않은 command 결과는 정적 검토와 구분합니다.
...
| --- | --- | --- | --- | --- |
| `Route-aware metadata and canonical identity`의 핵심 결정은 한 owner가 수행합니다. |  |  |  |  |
