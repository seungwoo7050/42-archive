...
JSON-LD script context를 깨뜨리지 않는 serializer와 site/person/project Schema.org graph가 production content에만 노출되는 과정을 복원합니다.

### 계획된 핵심 invariant

- `Safe JSON-LD serialization and structured claims`의 주요 결정은 route/design/component마다 중복 해석되지 않고 명시된 owner에 위치합니다.
...
| ---: | --- | --- | :---: | --- | --- |
| 1 | `228e40a48d64` | feat(seo): JSON-LD 안전 직렬화 경계 추가 | A | SEO | 초기 상태와 vocabulary를 고정합니다. |
| 2 | `ee98415be696` | feat(seo): 사이트 소유자 JSON-LD 모델 추가 | B | SEO | 기능·책임 경계를 확장합니다. |
| 3 | `ae4ec172e45a` | feat(seo): production layout에 사이트 JSON-LD 연결 | B | SEO | 기능·책임 경계를 확장합니다. |
| 4 | `7e09745d409e` | feat(seo): 프로젝트 CreativeWork JSON-LD 모델 추가 | B | SEO | 기능·책임 경계를 확장합니다. |
| 5 | `f7bd33a8b403` | feat(seo): 프로젝트 상세에 JSON-LD 연결 | B | SEO | 기능·책임 경계를 확장합니다. |
| 6 | `c5938ea4b4f8` | test(seo): JSON-LD 계약과 직렬화 검증 | A | VALIDATION, SEO, TEST | Thread의 통합·검증 상태를 확인합니다. |

## 5. Commit별 학습 기록

각 section은 반드시 해당 SHA의 tree와 parent diff를 기준으로 작성합니다. 같은 commit이 다른 확장 thread에 다시 등장해도 이 thread의 관점에서 별도로 확인합니다.

### 1. `228e40a48d64` — feat(seo): JSON-LD 안전 직렬화 경계 추가
...
- `228e40a48d64^`와 `228e40a48d64`의 first-parent diff에서 변경 파일과 핵심 symbol을 확인합니다.
...
- 먼저 `228e40a48d64^`와 `228e40a48d64`를 비교합니다.
...
### 2. `ee98415be696` — feat(seo): 사이트 소유자 JSON-LD 모델 추가
...
- `ee98415be696^`와 `ee98415be696`의 first-parent diff에서 변경 파일과 핵심 symbol을 확인합니다.
- Resulting tree에서 새 symbol의 caller/callee와 data/state ownership을 추적합니다.
- Commit이 추가한 입력, 출력, optional/disabled/unknown state와 integration point를 확인합니다.
- 이 SHA가 보장하는 범위와 후속 commit에 남긴 미완성 범위를 기록합니다.
...
- 먼저 `ee98415be696^`와 `ee98415be696`를 비교합니다.
- Final HEAD의 helper, test, file layout을 이 commit에 소급하지 않습니다.
- 실행하지 않은 command 결과는 정적 검토와 구분합니다.
...
### 3. `ae4ec172e45a` — feat(seo): production layout에 사이트 JSON-LD 연결
...
- `ae4ec172e45a^`와 `ae4ec172e45a`의 first-parent diff에서 변경 파일과 핵심 symbol을 확인합니다.
- Resulting tree에서 새 symbol의 caller/callee와 data/state ownership을 추적합니다.
- Commit이 추가한 입력, 출력, optional/disabled/unknown state와 integration point를 확인합니다.
- 이 SHA가 보장하는 범위와 후속 commit에 남긴 미완성 범위를 기록합니다.
...
- 먼저 `ae4ec172e45a^`와 `ae4ec172e45a`를 비교합니다.
- Final HEAD의 helper, test, file layout을 이 commit에 소급하지 않습니다.
- 실행하지 않은 command 결과는 정적 검토와 구분합니다.
...
### 4. `7e09745d409e` — feat(seo): 프로젝트 CreativeWork JSON-LD 모델 추가
...
- `7e09745d409e^`와 `7e09745d409e`의 first-parent diff에서 변경 파일과 핵심 symbol을 확인합니다.
- Resulting tree에서 새 symbol의 caller/callee와 data/state ownership을 추적합니다.
- Commit이 추가한 입력, 출력, optional/disabled/unknown state와 integration point를 확인합니다.
- 이 SHA가 보장하는 범위와 후속 commit에 남긴 미완성 범위를 기록합니다.
...
- 먼저 `7e09745d409e^`와 `7e09745d409e`를 비교합니다.
- Final HEAD의 helper, test, file layout을 이 commit에 소급하지 않습니다.
- 실행하지 않은 command 결과는 정적 검토와 구분합니다.
...
### 5. `f7bd33a8b403` — feat(seo): 프로젝트 상세에 JSON-LD 연결
...
- `f7bd33a8b403^`와 `f7bd33a8b403`의 first-parent diff에서 변경 파일과 핵심 symbol을 확인합니다.
- Resulting tree에서 새 symbol의 caller/callee와 data/state ownership을 추적합니다.
- Commit이 추가한 입력, 출력, optional/disabled/unknown state와 integration point를 확인합니다.
- 이 SHA가 보장하는 범위와 후속 commit에 남긴 미완성 범위를 기록합니다.
...
- 먼저 `f7bd33a8b403^`와 `f7bd33a8b403`를 비교합니다.
- Final HEAD의 helper, test, file layout을 이 commit에 소급하지 않습니다.
- 실행하지 않은 command 결과는 정적 검토와 구분합니다.
...
### 6. `c5938ea4b4f8` — test(seo): JSON-LD 계약과 직렬화 검증
...
- `c5938ea4b4f8^`와 `c5938ea4b4f8`의 first-parent diff에서 변경 파일과 핵심 symbol을 확인합니다.
- Resulting tree에서 새 symbol의 caller/callee와 data/state ownership을 추적합니다.
...
- 먼저 `c5938ea4b4f8^`와 `c5938ea4b4f8`를 비교합니다.
- Final HEAD의 helper, test, file layout을 이 commit에 소급하지 않습니다.
- 실행하지 않은 command 결과는 정적 검토와 구분합니다.
...
| --- | --- | --- | --- | --- |
| `Safe JSON-LD serialization and structured claims`의 핵심 결정은 한 owner가 수행합니다. |  |  |  |  |
