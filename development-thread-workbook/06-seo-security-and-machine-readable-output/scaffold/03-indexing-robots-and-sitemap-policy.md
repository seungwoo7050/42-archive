...
Template/production mode, enabled routes/projects와 verified origin이 metadata indexing, robots와 sitemap에 일관되게 적용되는 과정을 복원합니다.

### 계획된 핵심 invariant

- `Indexing, robots, and sitemap policy`의 주요 결정은 route/design/component마다 중복 해석되지 않고 명시된 owner에 위치합니다.
...
| ---: | --- | --- | :---: | --- | --- |
| 1 | `55b6061e0052` | feat(seo): 콘텐츠 mode별 metadata 정책 추가 | A | CONTENT, SEO | 초기 상태와 vocabulary를 고정합니다. |
| 2 | `cb61450ad922` | feat(seo): 콘텐츠 mode별 robots 정책 추가 | A | CONTENT, SEO | 기능·책임 경계를 확장합니다. |
| 3 | `67aabeab1553` | feat(seo): layout metadata를 콘텐츠 mode에 연결 | A | CONTENT, SEO | 기능·책임 경계를 확장합니다. |
| 4 | `70b69f04e8c7` | feat(seo): 공개 route sitemap 생성 | A | ARCH, ROUTING, SEO | 기능·책임 경계를 확장합니다. |
| 5 | `fb3d18fd660b` | test(content): readiness와 indexing 계약 검증 | A | CONTENT, VALIDATION, SEO | 기능·책임 경계를 확장합니다. |
| 6 | `166f05f7be06` | test(e2e): 콘텐츠 mode별 metadata와 robots 검증 | A | CONTENT, VALIDATION, SEO | Thread의 통합·검증 상태를 확인합니다. |

## 5. Commit별 학습 기록

각 section은 반드시 해당 SHA의 tree와 parent diff를 기준으로 작성합니다. 같은 commit이 다른 확장 thread에 다시 등장해도 이 thread의 관점에서 별도로 확인합니다.

### 1. `55b6061e0052` — feat(seo): 콘텐츠 mode별 metadata 정책 추가
...
- `55b6061e0052^`와 `55b6061e0052`의 first-parent diff에서 변경 파일과 핵심 symbol을 확인합니다.
...
- 먼저 `55b6061e0052^`와 `55b6061e0052`를 비교합니다.
...
### 2. `cb61450ad922` — feat(seo): 콘텐츠 mode별 robots 정책 추가
...
- `cb61450ad922^`와 `cb61450ad922`의 first-parent diff에서 변경 파일과 핵심 symbol을 확인합니다.
- Resulting tree에서 새 symbol의 caller/callee와 data/state ownership을 추적합니다.
- Commit이 추가한 입력, 출력, optional/disabled/unknown state와 integration point를 확인합니다.
- 이 SHA가 보장하는 범위와 후속 commit에 남긴 미완성 범위를 기록합니다.
...
- 먼저 `cb61450ad922^`와 `cb61450ad922`를 비교합니다.
- Final HEAD의 helper, test, file layout을 이 commit에 소급하지 않습니다.
- 실행하지 않은 command 결과는 정적 검토와 구분합니다.
...
### 3. `67aabeab1553` — feat(seo): layout metadata를 콘텐츠 mode에 연결
...
- `67aabeab1553^`와 `67aabeab1553`의 first-parent diff에서 변경 파일과 핵심 symbol을 확인합니다.
- Resulting tree에서 새 symbol의 caller/callee와 data/state ownership을 추적합니다.
- Commit이 추가한 입력, 출력, optional/disabled/unknown state와 integration point를 확인합니다.
- 이 SHA가 보장하는 범위와 후속 commit에 남긴 미완성 범위를 기록합니다.
...
- 먼저 `67aabeab1553^`와 `67aabeab1553`를 비교합니다.
- Final HEAD의 helper, test, file layout을 이 commit에 소급하지 않습니다.
- 실행하지 않은 command 결과는 정적 검토와 구분합니다.
...
### 4. `70b69f04e8c7` — feat(seo): 공개 route sitemap 생성
...
- `70b69f04e8c7^`와 `70b69f04e8c7`의 first-parent diff에서 변경 파일과 핵심 symbol을 확인합니다.
- Resulting tree에서 새 symbol의 caller/callee와 data/state ownership을 추적합니다.
- Commit이 추가한 입력, 출력, optional/disabled/unknown state와 integration point를 확인합니다.
- 이 SHA가 보장하는 범위와 후속 commit에 남긴 미완성 범위를 기록합니다.
...
- 먼저 `70b69f04e8c7^`와 `70b69f04e8c7`를 비교합니다.
- Final HEAD의 helper, test, file layout을 이 commit에 소급하지 않습니다.
- 실행하지 않은 command 결과는 정적 검토와 구분합니다.
...
### 5. `fb3d18fd660b` — test(content): readiness와 indexing 계약 검증
...
- `fb3d18fd660b^`와 `fb3d18fd660b`의 first-parent diff에서 변경 파일과 핵심 symbol을 확인합니다.
- Resulting tree에서 새 symbol의 caller/callee와 data/state ownership을 추적합니다.
...
- 먼저 `fb3d18fd660b^`와 `fb3d18fd660b`를 비교합니다.
- Final HEAD의 helper, test, file layout을 이 commit에 소급하지 않습니다.
- 실행하지 않은 command 결과는 정적 검토와 구분합니다.
...
### 6. `166f05f7be06` — test(e2e): 콘텐츠 mode별 metadata와 robots 검증
...
- `166f05f7be06^`와 `166f05f7be06`의 first-parent diff에서 변경 파일과 핵심 symbol을 확인합니다.
- Resulting tree에서 새 symbol의 caller/callee와 data/state ownership을 추적합니다.
- 대상 production invariant, fixture/failure injection, test technique와 실제 production path를 구분합니다.
- Test가 증명하는 것과 증명하지 않는 것을 명시합니다.
...
- 먼저 `166f05f7be06^`와 `166f05f7be06`를 비교합니다.
- Final HEAD의 helper, test, file layout을 이 commit에 소급하지 않습니다.
- 실행하지 않은 command 결과는 정적 검토와 구분합니다.
...
| --- | --- | --- | --- | --- |
| `Indexing, robots, and sitemap policy`의 핵심 결정은 한 owner가 수행합니다. |  |  |  |  |
