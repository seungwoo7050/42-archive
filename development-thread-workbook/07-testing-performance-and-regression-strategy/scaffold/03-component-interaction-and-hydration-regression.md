...
Design selector와 project links의 interaction contract, native `<details>` hydration race와 server/client boundary regression을 결정적으로 검증하는 과정을 복원합니다.

### 계획된 핵심 invariant

- `Component interaction and hydration regression`의 주요 결정은 route/design/component마다 중복 해석되지 않고 명시된 owner에 위치합니다.
...
| ---: | --- | --- | :---: | --- | --- |
| 1 | `09cec616f314` | test(ui): 디자인 선택과 프로젝트 링크 계약 검증 | A | VALIDATION, TEST | 초기 상태와 vocabulary를 고정합니다. |
| 2 | `c702b870d57a` | fix(ui): hydration 중 native details 상태 보존 | A | DEBUG | 기능·책임 경계를 확장합니다. |
| 3 | `b6c0238ab8b8` | test(ui): details hydration 경쟁 조건 검증 | A | VALIDATION, TEST | 기능·책임 경계를 확장합니다. |
| 4 | `a37cb8596733` | refactor(ui): 디자인 선택기를 server markup으로 전환 | A | ARCH, REFACTOR | 기능·책임 경계를 확장합니다. |
| 5 | `1ac7813155c6` | test(ui): server 선택기와 focus 복원 검증 | A | VALIDATION, A11Y, TEST | Thread의 통합·검증 상태를 확인합니다. |

## 5. Commit별 학습 기록

각 section은 반드시 해당 SHA의 tree와 parent diff를 기준으로 작성합니다. 같은 commit이 다른 확장 thread에 다시 등장해도 이 thread의 관점에서 별도로 확인합니다.

### 1. `09cec616f314` — test(ui): 디자인 선택과 프로젝트 링크 계약 검증
...
- `09cec616f314^`와 `09cec616f314`의 first-parent diff에서 변경 파일과 핵심 symbol을 확인합니다.
...
- 먼저 `09cec616f314^`와 `09cec616f314`를 비교합니다.
...
### 2. `c702b870d57a` — fix(ui): hydration 중 native details 상태 보존
...
- `c702b870d57a^`와 `c702b870d57a`의 first-parent diff에서 변경 파일과 핵심 symbol을 확인합니다.
- Resulting tree에서 새 symbol의 caller/callee와 data/state ownership을 추적합니다.
- 수정 전 가정, 실제 failure/risk, root cause, corrected invariant를 분리해 기록합니다.
- 동일 failure를 재현하거나 막는 regression evidence가 같은 SHA 또는 후속 SHA에 있는지 확인합니다.

확인 원칙:

- 먼저 `c702b870d57a^`와 `c702b870d57a`를 비교합니다.
- Final HEAD의 helper, test, file layout을 이 commit에 소급하지 않습니다.
- 실행하지 않은 command 결과는 정적 검토와 구분합니다.
...
### 3. `b6c0238ab8b8` — test(ui): details hydration 경쟁 조건 검증
...
- `b6c0238ab8b8^`와 `b6c0238ab8b8`의 first-parent diff에서 변경 파일과 핵심 symbol을 확인합니다.
- Resulting tree에서 새 symbol의 caller/callee와 data/state ownership을 추적합니다.
- 대상 production invariant, fixture/failure injection, test technique와 실제 production path를 구분합니다.
- Test가 증명하는 것과 증명하지 않는 것을 명시합니다.
...
- 먼저 `b6c0238ab8b8^`와 `b6c0238ab8b8`를 비교합니다.
- Final HEAD의 helper, test, file layout을 이 commit에 소급하지 않습니다.
- 실행하지 않은 command 결과는 정적 검토와 구분합니다.
...
### 4. `a37cb8596733` — refactor(ui): 디자인 선택기를 server markup으로 전환
...
- `a37cb8596733^`와 `a37cb8596733`의 first-parent diff에서 변경 파일과 핵심 symbol을 확인합니다.
- Resulting tree에서 새 symbol의 caller/callee와 data/state ownership을 추적합니다.
...
- 먼저 `a37cb8596733^`와 `a37cb8596733`를 비교합니다.
- Final HEAD의 helper, test, file layout을 이 commit에 소급하지 않습니다.
- 실행하지 않은 command 결과는 정적 검토와 구분합니다.
...
### 5. `1ac7813155c6` — test(ui): server 선택기와 focus 복원 검증
...
- `1ac7813155c6^`와 `1ac7813155c6`의 first-parent diff에서 변경 파일과 핵심 symbol을 확인합니다.
- Resulting tree에서 새 symbol의 caller/callee와 data/state ownership을 추적합니다.
- 대상 production invariant, fixture/failure injection, test technique와 실제 production path를 구분합니다.
- Test가 증명하는 것과 증명하지 않는 것을 명시합니다.
...
- 먼저 `1ac7813155c6^`와 `1ac7813155c6`를 비교합니다.
- Final HEAD의 helper, test, file layout을 이 commit에 소급하지 않습니다.
- 실행하지 않은 command 결과는 정적 검토와 구분합니다.
...
| --- | --- | --- | --- | --- |
| `Component interaction and hydration regression`의 핵심 결정은 한 owner가 수행합니다. |  |  |  |  |
