...
Desktop/mobile Chromium에서 enabled route×five-design matrix, landmarks, Axe WCAG tags와 keyboard skip path를 검증하는 broad integration strategy를 복원합니다.

### 계획된 핵심 invariant

- `Browser accessibility route matrix`의 주요 결정은 route/design/component마다 중복 해석되지 않고 명시된 owner에 위치합니다.
...
| ---: | --- | --- | :---: | --- | --- |
| 1 | `31c438b52e4b` | test(e2e): 다섯 디자인의 route matrix 검증 | A | ARCH, VALIDATION, ROUTING | 초기 상태와 vocabulary를 고정합니다. |
| 2 | `5a6fd8a802ff` | fix(a11y): 디자인별 색상 대비 보정 | A | A11Y, DEBUG | 기능·책임 경계를 확장합니다. |
| 3 | `a15e117cb51b` | fix(a11y): skip link focus target 복원 | A | A11Y, DEBUG | 기능·책임 경계를 확장합니다. |
| 4 | `e1aac08e0e9e` | fix(a11y): Brutalist 지표의 definition semantics 수정 | A | ARCH, RENDERER, A11Y | 기능·책임 경계를 확장합니다. |
| 5 | `84c71d027630` | test(a11y): 디자인×route WCAG 행렬 추가 | A | ARCH, ROUTING, A11Y | Thread의 통합·검증 상태를 확인합니다. |
...
- `5a6fd8a802ff^`와 `5a6fd8a802ff`의 first-parent diff에서 변경 파일과 핵심 symbol을 확인합니다.
- Resulting tree에서 새 symbol의 caller/callee와 data/state ownership을 추적합니다.
...
- 먼저 `5a6fd8a802ff^`와 `5a6fd8a802ff`를 비교합니다.
- Final HEAD의 helper, test, file layout을 이 commit에 소급하지 않습니다.
- 실행하지 않은 command 결과는 정적 검토와 구분합니다.
...
- `a15e117cb51b^`와 `a15e117cb51b`의 first-parent diff에서 변경 파일과 핵심 symbol을 확인합니다.
- Resulting tree에서 새 symbol의 caller/callee와 data/state ownership을 추적합니다.
- 수정 전 가정, 실제 failure/risk, root cause, corrected invariant를 분리해 기록합니다.
- 동일 failure를 재현하거나 막는 regression evidence가 같은 SHA 또는 후속 SHA에 있는지 확인합니다.
...
- 먼저 `a15e117cb51b^`와 `a15e117cb51b`를 비교합니다.
- Final HEAD의 helper, test, file layout을 이 commit에 소급하지 않습니다.
- 실행하지 않은 command 결과는 정적 검토와 구분합니다.
...
- `e1aac08e0e9e^`와 `e1aac08e0e9e`의 first-parent diff에서 변경 파일과 핵심 symbol을 확인합니다.
- Resulting tree에서 새 symbol의 caller/callee와 data/state ownership을 추적합니다.
- 수정 전 가정, 실제 failure/risk, root cause, corrected invariant를 분리해 기록합니다.
- 동일 failure를 재현하거나 막는 regression evidence가 같은 SHA 또는 후속 SHA에 있는지 확인합니다.
...
- 먼저 `e1aac08e0e9e^`와 `e1aac08e0e9e`를 비교합니다.
- Final HEAD의 helper, test, file layout을 이 commit에 소급하지 않습니다.
- 실행하지 않은 command 결과는 정적 검토와 구분합니다.
...
- `84c71d027630^`와 `84c71d027630`의 first-parent diff에서 변경 파일과 핵심 symbol을 확인합니다.
- Resulting tree에서 새 symbol의 caller/callee와 data/state ownership을 추적합니다.
- 대상 production invariant, fixture/failure injection, test technique와 실제 production path를 구분합니다.
- Test가 증명하는 것과 증명하지 않는 것을 명시합니다.
...
- 먼저 `84c71d027630^`와 `84c71d027630`를 비교합니다.
- Final HEAD의 helper, test, file layout을 이 commit에 소급하지 않습니다.
- 실행하지 않은 command 결과는 정적 검토와 구분합니다.
...
| --- | --- | --- | --- | --- |
| `Browser accessibility route matrix`의 핵심 결정은 한 owner가 수행합니다. |  |  |  |  |
